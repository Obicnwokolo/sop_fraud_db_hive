...
object incrload_fraud {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(s"USE bigdata_nov_2024")

    val hiveTable = "sop_fraud_trans"
    val maxIdHive = spark.sql(s"SELECT MAX(CAST(id AS BIGINT)) AS max_id FROM $hiveTable")
      .first()
      .getAs[Long]("max_id")
